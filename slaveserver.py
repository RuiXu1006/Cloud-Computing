...
server = [1]*serverNum
serverListener = [1] * serverNum

#masterserver = MasterServer()
#masterserver.start()

# helper class for the svr to master listener thread to start
class SvrToMaster(Thread):
    def __init__(self,id):
        Thread.__init__(self)
        self.id =id
...
# helper class for the svr to client listener thread to start
class SvrToClient(Thread):
    def __init__(self,id):
        Thread.__init__(self)
        self.id =id
    def run(self):
...
# for i in range(1,serverNum):
#     helper1[i] = SvrToMaster(i)
#     helper1[i].start()
#     helper2[i] = SvrToClient(i)
#     helper2[i].start()